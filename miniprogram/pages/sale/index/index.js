...
Page({
  ...
  onShow:function(){
    try {
      ...
      http.getNewInbox(type, res => {
        try {
          if (res && res.data && res.data.data != 0 && res.data.data != null && res.data.data != '') {
            this.setData({
              newMessage: true,
  ...
  comment:function(e){
    ...
    wx.navigateTo({
      url: '/pages/sale/comment/comment?id='+id
    })
  },
  ...
  getList:function(){
    try {
              ...
              data.map(item => {
                if(item.attachments && item.attachments.length>=1){
                  if (leftHeight <= rightHeigt) {
                    leftList.push(item);
                    leftHeight += item.attachments[0]['height'] || 200;
                  } else {
                    rightList.push(item)
                    rightHeigt += item.attachments[0]['height'] || 200;
                  }
                  this.setData({
                    leftList: leftList,
                    rightList: rightList,
      ...
      console.error('getList方法执行出错:', error);
      wx.hideLoading();
      this.setData({
        showGeMoreLoadin: false,
        notDataTips: true
